...
#line 1 "/home/vishrut/Study/USC/Sem-1/CSCI 596: Scientific Computing and Visualization/project/mlflip-mod/source/fileio/iomeshes.cpp"
/******************************************************************************
 *
...
void writeBobjFile(const string& name, const BasicParticleSystem* pp, const ParticleDataImpl<int> *pT, const int exclude) {
	debMsg( "writing particle mesh file " << name ,1);
#	if NO_ZLIB!=1
	const Real dx = pp->getParent()->getDx();
	const Vector3D<float> gs = toVec3f(pp->getParent()->getGridSize());
	...
	for(int i=0; i<pp->size(); ++i) {
		...
		const Vector3D<float> pos = (toVec3f(pp->getPos(i)) - gs*0.5)*dx; // normalize to unit cube around 0
		gzwrite(gzf, &pos.value[0], sizeof(float)*3);
	}
