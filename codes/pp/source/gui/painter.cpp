...
#line 1 "/home/vishrut/Study/USC/Sem-1/CSCI 596: Scientific Computing and Visualization/project/mlflip-mod/source/gui/painter.cpp"
/******************************************************************************
 *
...
void GridPainter<T>::processKeyEvent(PainterEvent e, int param)
{
	if (e == EventSetDim) {
		mDim = param;
		if (mLocalGrid && !mLocalGrid->is3D()) mDim = 2;
...
template<> void GridPainter<int>::paint() {
	...
	bool drawLines = true;	// If you don't want lines, just turn off
	if (drawLines) {
		//glDepthFunc(GL_LESS);
		glBegin(GL_LINES);
