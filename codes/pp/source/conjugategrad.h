...
#line 1 "/home/vishrut/Study/USC/Sem-1/CSCI 596: Scientific Computing and Visualization/project/mlflip-mod/source/conjugategrad.h"
/******************************************************************************
 *
...
//! Kernel: Construct the matrix for the poisson equation
 ...
 struct MakeLaplaceMatrix : public KernelBase { MakeLaplaceMatrix(const FlagGrid &flags, Grid<Real> &A0, Grid<Real> &Ai, Grid<Real> &Aj, Grid<Real> &Ak, const MACGrid *fractions=NULL) :  KernelBase(&flags,1) ,flags(flags),A0(A0),Ai(Ai),Aj(Aj),Ak(Ak),fractions(fractions)   { runMessage(); run(); }  inline void op(int i, int j, int k, const FlagGrid &flags, Grid<Real> &A0, Grid<Real> &Ai, Grid<Real> &Aj, Grid<Real> &Ak, const MACGrid *fractions=NULL )  {
