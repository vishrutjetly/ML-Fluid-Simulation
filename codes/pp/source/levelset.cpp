...
#line 1 "/home/vishrut/Study/USC/Sem-1/CSCI 596: Scientific Computing and Visualization/project/mlflip-mod/source/levelset.cpp"
/******************************************************************************
 *
...
//! Kernel: perform levelet intersection
 struct KnIntersect : public KernelBase { KnIntersect(Grid<Real>& a, const Grid<Real>& b) :  KernelBase(&a,0) ,a(a),b(b)   { runMessage(); run(); }   inline void op(IndexInt idx, Grid<Real>& a, const Grid<Real>& b )  {
	a[idx] = max(a[idx], b[idx]);
}    inline Grid<Real>& getArg0() { return a; } typedef Grid<Real> type0;inline const Grid<Real>& getArg1() { return b; } typedef Grid<Real> type1; void runMessage() { debMsg("Executing kernel KnIntersect ", 3); debMsg("Kernel range" <<  " x "<<  maxX  << " y "<< maxY  << " z "<< minZ<<" - "<< maxZ  << " "   , 4); }; void run() {   const IndexInt _sz = size; 
#pragma omp parallel 
 {  
#pragma omp for  
  for (IndexInt i = 0; i < _sz; i++) op(i,a,b);  }   } Grid<Real>& a; const Grid<Real>& b;   };
#line 119 "levelset.cpp"
