...
parser.add_argument(	  '--epoch',	  default="",  				help='Epoch Number')

pargs = parser.parse_known_args()[0]

if pargs.load is None: 
	sys.exit('You have to specify the path to the trained model.')
...
model = tf.keras.models.load_model(pargs.load+"/model-" + str(pargs.epoch))

pathsplit = "/".join(pargs.load.split("/")[2:])


image_path = "images/"+pathsplit+"/"+str(pargs.epoch)+"epochs/"
