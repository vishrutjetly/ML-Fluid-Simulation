    ...
    with tf.name_scope(layer_name):
        ...
        with tf.name_scope('Wx_plus_b' if not bn else 'BN_Wx_plus_b'):
            if bn:
                ...
                epsilon =  1e-4
                z_bn        = tf.matmul(input_tensor, weights if keep_prob is None else tf.nn.dropout(weights, rate=1-keep_prob))
                ...
                preactivate = tf.matmul(input_tensor, weights if keep_prob is None else tf.nn.dropout(weights, rate=1-keep_prob)) + biases
